def calculate_average_temperature(weather_data,x):
    # Calculate the average temperature for the month
    total_temperature = sum(weather_data[x])
    num_days = len(weather_data[x])
...
average_temperature = calculate_average_temperature(weather_data,'temperature')
